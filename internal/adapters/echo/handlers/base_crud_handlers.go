...
type BaseCRUDHandlers[T any] interface {
	RegisterRoutes(e *echo.Group)
	GetPath() string
	GetServices() crud_services.BaseCRUDService[T]
}

type baseCRUDHandlers[T any] struct {
...
func NewBaseCRUDHandlers[T any](services crud_services.BaseCRUDService[T]) BaseCRUDHandlers[T] {
	ent := services.GetEntity()
	return &baseCRUDHandlers[T]{
		services: services,
		title:    ent.GetTitle(),
...
func (h baseCRUDHandlers[T]) RegisterRoutes(e *echo.Group) {
...
func (h baseCRUDHandlers[T]) GetServices() crud_services.BaseCRUDService[T] {
	return h.services
}

func (h baseCRUDHandlers[T]) GetPath() string {
	return h.path
}
...
func (h baseCRUDHandlers[T]) getPagination(c echo.Context) error {
...
// POST {path}/create
func (h baseCRUDHandlers[T]) actCreate(c echo.Context) error {
	var req T
...
// GET {path}/:id/show
func (h baseCRUDHandlers[T]) getShow(c echo.Context) error {
...
func (h baseCRUDHandlers[T]) actUpdate(c echo.Context) error {
	id, err := validateUUID(c)
	if err != nil {
		return respErr(c, http.StatusBadRequest, err)
	}
...
func (h baseCRUDHandlers[T]) actRemove(c echo.Context) error {
	id, err := validateUUID(c)
	if err != nil {
		return respErr(c, http.StatusBadRequest, err)
	}
